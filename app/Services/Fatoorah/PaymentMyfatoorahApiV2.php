...
namespace App\Services\Fatoorah;
    ...
    public function getPaymentStatus($keyId, $KeyType, $orderId = null)
    {
        ...
        if ($orderId && $json->Data->CustomerReference != $orderId) {
            return [
                ...
                'success'    => false,
                'message'    => 'Trying to call data of another order',
                'code'       => 10001
            ];

            // throw new \Exception('Trying to call data of another order', 10001);
            ...
            // throw new \Exception('Duplicate Payment', 10002); //success with Duplicate
        }
            ...
            foreach ($json->Data->InvoiceTransactions as $transaction) {
                if ($transaction->PaymentId == $keyId && $transaction->Error && $json->Data->InvoiceStatus != 'Paid') {
                    return [
                        'invoice_id' => $json->Data->InvoiceId,
                        'success'    => false,
                        'message'    => 'Failed with Error (' . $transaction->Error . ')',
                        'code'       => 10003
                    ];

                    // throw new \Exception('Failed with Error (' . $transaction->Error . ')', 10003); //faild order
                }
            }
        }
        ...
        if ($json->Data->InvoiceStatus != 'Paid') {
            ...
            if ($lastInvoiceTransactions && $lastInvoiceTransactions->Error) {
                return [
                    'invoice_id' => $json->Data->InvoiceId,
                    'success'    => false,
                    'message'    => 'Failed with Error (' . $lastInvoiceTransactions->Error . ')',
                    'code'       => 10004
                ];

                // throw new \Exception('Failed with Error (' . $lastInvoiceTransactions->Error . ')', 10004); //faild order
            }
            ...
            if ($ExpiryDate < $currentDate) {
                return [
                    'invoice_id' => $json->Data->InvoiceId,
                    'success'    => false,
                    'message'    => 'Invoice is expired since: ' . $ExpiryDate->format('Y-m-d'),
                    'code'       => 10005
                ];

                // throw new \Exception('Invoice is expired since: ' . $ExpiryDate->format('Y-m-d'), 10005); //cancelled order
            }
            ...
            // throw new \Exception('Payment is pending', 10006);
        }
